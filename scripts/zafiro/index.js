...
const CHECKBOX_IDS = [
  ...
  {
    checkboxId: "ZAFIRO_WRK_FLAG6",
    id: 6,
    name: "Empleados Completo",
  },
  {
    checkboxId: "ZAFIRO_WRK_FLAG_LM",
    id: 0,
    name: "Historial Posición"
  }
];
...
async function waitForNewDownload(dir, previos, timeoutMs = 900000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const nuevosCompletos = actuales.filter(
      (f) => !previos.has(f) && f.endsWith(".csv") && !f.startsWith(".~lock") && !f.startsWith("zafiro_info_"),
    );
...
const main = async () => {
  ...
  try {
    ...
    for (const item of CHECKBOX_IDS) {
      try {
        await driver.executeScript(`
          var cb = document.getElementById('${item.checkboxId}');
          if (cb && cb.checked) {
            cb.click();
          }
        `);
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(__dirname, "error_screenshot.png"),
        screenshot,
        "base64",
      );
